...
export async function POST(request: NextRequest) {
  try {
    ...
    // Debug: log what we're sending to Ultravox
    const callPayload = { ...body, recordingEnabled: true };
    console.log('🔧 Sending to Ultravox:', JSON.stringify(callPayload, null, 2));
    console.log('🔧 selectedTools in payload:', callPayload.selectedTools?.length || 'NONE');
