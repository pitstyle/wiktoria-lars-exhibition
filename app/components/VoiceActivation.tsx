...
export default function VoiceActivation({
  ...
  const vadCallbacks = React.useMemo(() => ({
    onVoiceStart: () => {
      ...
      // Trigger call activation on first voice detection
      if (!hasTriggeredActivation.current && enabled) {
    ...
    onSilenceTimeout: () => {
      ...
      // Reset activation trigger for next voice detection
      hasTriggeredActivation.current = false;
    }
  }), [onVoiceStart, onVoiceActivation, enabled, onVoiceEnd]);
      ...
      const freshCallbacks = {
        onVoiceStart: () => {
          ...
          if (!hasTriggeredActivation.current && enabled) {
            ...
            console.log('🚀 NOT triggering - already triggered or disabled');
          }
        },
        onVoiceEnd: () => {
          console.log('🎤 Voice activity ended');
