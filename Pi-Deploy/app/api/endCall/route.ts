    ...
    if (callId) {
      ...
      if (findError || !conversations) {
        ...
        if (fullTranscript?.results?.length > 0) {
          console.log('🎯 Attempting to extract topic from transcript...');
          try {
            ...
            if (topicResponse.ok) {
              ...
              console.log('✅ Topic extraction result:', topicResult);
            } else {
              console.warn('⚠️ Topic extraction failed:', topicResponse.status);
            }
          } catch (topicError) {
            console.warn('⚠️ Topic extraction error:', (topicError as Error).message);
          }
        }
      }
    } else {
