...
export default function ExhibitionInterface({
  ...
  const resetUserSilenceTimeout = useCallback((forceActive = false) => {
    ...
    if (effectiveIsCallActive && userSilenceTimeout && userHasResponded) {
      ...
      const checkForSilence = () => {
        const timeSinceLastSpeech = Date.now() - lastAnySpeechTime;
        console.log(`🔍 Checking silence: ${timeSinceLastSpeech}ms since last speech`);
    ...
    const handleSilenceTimeout = async () => {
      try {
        ...
        onSessionEnd?.();
        console.log('✅ Exhibition session ended due to true silence, returning to waiting state for next user');
  ...
  const returnToWaitingState = useCallback(async (reason: string = 'unknown') => {
    console.log(`🔄 Returning to waiting state (reason: ${reason})`);
    ...
    try {
      ...
      onSessionEnd?.();
      console.log('✅ Exhibition session ended, returning to waiting state for next user');
    } catch (error) {
  ...
  const handleStatusChange = useCallback(async (status: UltravoxSessionStatus | string | undefined) => {
    ...
    if (status) {
      ...
      if ((status === 'disconnected' || status === 'disconnecting') && isCallActive) {
        console.log('🔌 Ultravox session ended naturally - returning to waiting state');
  ...
  }, [resetSessionTimeout, isCallActive, returnToWaitingState]);
  ...
  }, [resetSessionTimeout, resetUserSilenceTimeout]);
